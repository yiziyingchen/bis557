...
`%*%.sparse.matrix` <- function(a,b){
  ...
  a1 <- data.frame(i = a$i, j = a$j, x = a$x)
  b1 <- data.frame(i = b$i, j = b$j, x = b$x)
  c <- merge(index, a1, by = c("i", "j"), all.x = TRUE, sort = FALSE)
  c1 <- merge(c, b1, by = c("i", "j"), all = TRUE, suffixes = c("1", "2"))
  c1$x1[is.na(c1$x1)]<- 0
  c1$x2[is.na(c1$x2)]<- 0
  ...
  final<- sparse.matrix(i = c3$i, j = c3$j, x = c3$x, dims = c(max(c3$i), max(c3$j)))
  final
}
...
sm1 %*% sm3 == sparse.matrix(i = rep(1:2, 3), j = rep(1:3, each = 2),
